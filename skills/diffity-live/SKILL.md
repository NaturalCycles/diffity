...
The reader asked something in the review page and `diffity agent await` handed it to you. Answer
it, then go back to waiting.

**This is the tail of a review, not a review.** It assumes the findings and the reading order are
already in — /diffity-review does that, and arms this loop as its last step. If you have arrived here
...
```
diffity agent reply <thread-id> --aside --answers <comment-id> --body "<your answer>"
```
...
```
diffity agent reply <thread-id> --aside --answers <comment-id> --body-file - <<'EOF'
...
```
diffity agent amend <finding-comment-id> --body "<the rewritten finding>"
diffity agent reply <thread-id> --aside --answers <comment-id> --body "Rewritten."
```
...
```
# make the edit, then:
diffity agent reply <thread-id> --aside --answers <comment-id> --body "<what you changed>"
```
...
Run `diffity agent live-status` if you are unsure.
