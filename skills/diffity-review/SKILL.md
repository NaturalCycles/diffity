...
3. If **no session is running** for this repo, start one in the background:
   - Command: `diffity <ref> --no-open --review` (or `diffity --no-open --review` if no ref)
...
3. For each finding, leave an inline comment using:
   ```
   diffity agent comment --file <path> --line <n> [--end-line <n>] [--side new] --body-file - <<'EOF'
   <comment>
   EOF
   ```
...
Check whether the page can reach you: `diffity agent live-status`. If it reports that live mode is
...
exits, which is what wakes you; nothing asked exits 3, and you re-arm. Follow the **/diffity-live**
