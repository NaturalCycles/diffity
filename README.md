...
> ⚠️ The agent runs inside a checkout the pull request's author controls, so it can execute their repository code. The daemon runs it without the forge's credentials in its environment, without your Claude settings, and with the deny list above — but that is defence in depth, not a sandbox.
