...
export const SETTINGS_KEYS = ['filter', 'skipTitles', 'alertWhen', 'alertPaths', 'postAlerts', 'postPrefix', 'postSeverities', 'postFooter', 'quietOnceCommented', 'maxPrepared', 'pollMinutes', 'live', 'liveTimeoutMinutes', 'prepareTimeoutMinutes', 'waitForCi', 'agent', 'validate', 'triage'] as const;
...
export function parseSettingsPatch(body: string): SettingsPatch {
  ...
  for (const key of ['filter', 'alertWhen', 'postPrefix', 'postFooter'] as const) {
